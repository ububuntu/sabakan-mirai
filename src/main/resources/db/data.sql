...
-- 暗号 (規則に基づく変換)
('C_CRYP_001', '命令Aは「+1」、命令Bは「×2」を表す。「3」に対してA→Bの順に操作した結果はどれか。', '6', '7', '8', '9', '3', '暗号'),
('C_CRYP_002', '記号○は「1引く」、△は「3倍する」を表す。「5」に対して○→△→○の順に操作した結果はどれか。', '10', '11', '12', '14', '2', '暗号'),
('C_CRYP_003', '変換Xは「2で割る」、変換Yは「5足す」を表す。ある数にX→Yの順に操作したら「10」になった。元の数はどれか。', '8', '10', '12', '20', '2', '暗号'),
('C_CRYP_004', '暗号化ルール：数字を「+2」してアルファベット順に変換する（1→A, 2→B, 3→C...）。数字「1・3」を暗号化した結果はどれか。', 'A C', 'B D', 'C E', 'D F', '3', '暗号'),
('C_CRYP_005', '指令ア：「位を逆にする(12→21)」。指令イ：「間に0を入れる(12→102)」。数字「45」にア→イの順に操作した結果はどれか。', '405', '504', '450', '540', '2', '暗号');